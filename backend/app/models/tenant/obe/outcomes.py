...
class PerformanceIndicator(UUIDPKMixin, TimestampMixin, TenantBase):
    """Indicator-Based PO Definition Method only (spec §18) — a PO's
    sub-indicators, e.g. PO(a) -> PI(a1), PI(a2). `code` is generated by the
    service layer from the parent PO's code + `sequence` at creation time
    (spec §18: "must automatically inherit the PO identifier"), then stored
    verbatim here rather than recomputed on every read — same precedent as
    `CourseOutcome.code` being a stored, service-assigned value.

    schema="program": see `PEO`'s docstring above.
    """
